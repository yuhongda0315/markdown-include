 ...
 */
exports.compileFiles = function (options) {
	...
	for (i = 0; i < files.length; i += 1) {
		...
		if (options.tableOfContents) {
			...
			if (options.tableOfContents.heading && self.tableOfContents) {
				self.build[file].parsedData = options.tableOfContents.heading + '\n\n' + self.tableOfContents + '\n\n' + self.build[file].parsedData;
			}
		}
	}
